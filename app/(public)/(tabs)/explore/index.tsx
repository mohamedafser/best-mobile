import {
  BookingSearchBar,
  createDefaultBookingSearchValues,
  type BookingSearchValues,
} from "@/components/BookingSearchBar";
import { formatDate } from "@/lib/utils/dateHelpers";
import { buildHotelUrl, buildSearchUrl } from "@/lib/utils/url-builder";
...
const HotelCard = ({ item }: { item: Hotel }) => (
  <TouchableOpacity
    ...
    onPress={() => router.push("/hotel-details")}
  >
...
export default function Explore() {
  const [bookingSearchValues, setBookingSearchValues] =
    useState<BookingSearchValues>(createDefaultBookingSearchValues);

  const handleBookingSearchChange = useCallback((next: BookingSearchValues) => {
    setBookingSearchValues(next);
  }, []);

  const handleSearch = useCallback((values: BookingSearchValues) => {
    if (!values) return;
    let url: string = "";
    const { selectedHotel } = values;
    if (values?.selectedHotel) {
      if (
        !selectedHotel?.slug_details?.country_code ||
        !selectedHotel?.slug_details?.slug
      )
        return;
      url = buildHotelUrl(
        selectedHotel?.slug_details?.country_code || "",
        selectedHotel?.slug_details?.slug,
        {
          ...
          roomQuantity: values?.rooms || 1,
          childrenAges: values?.childrenAges || [],
          propertyId: String(
            selectedHotel?.hotel_id ||
              selectedHotel?.slug_details?.property_code ||
              "",
          ),
        },
      );
    ...
    if (!selectedHotel) {
      url = buildSearchUrl({
        ...
        roomQuantity: values?.rooms || 1,
        cityName: values?.destinationText || "",
        childrenAges: values?.childrenAges || [],
        stateUuid: values?.destination?.state?.uuid,
      });
    }
